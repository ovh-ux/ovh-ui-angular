    ...
    describe("Directive", () => {
        it("should create a tooltip, next to the trigger, with the attribute value as text", () => {
            const component = testUtils.compileTemplate(
                '<div><button class="trigger" oui-tooltip="foo"></button></div>'
            );

            $timeout.flush();

            const tooltip = angular
                .element(component[0].querySelector(".trigger"))
                .next();
        ...
        it("should create a tooltip, next to the trigger, with the attribute value as text", () => {
            const component = testUtils.compileTemplate(
                '<div><button class="trigger" title="foo" oui-tooltip></button></div>'
            );
        ...
        it("should not create a tooltip, if the provided text is empty", () => {
            const emptyText = " ";
            const component = testUtils.compileTemplate(
                `<div><button class="trigger" oui-tooltip="${emptyText}"`
            );
        ...
        it("should add an attribute aria-label, if there is none on the trigger", () => {
            ...
            const trigger = angular.element(
                component[0].querySelector(".trigger")
            );
        ...
        it("should keep the value of the attribute aria-label, if there is one on the trigger", () => {
            const component = testUtils.compileTemplate(
                '<div><button class="trigger" oui-tooltip="foo" aria-label="bar"></button></div>'
            );
        ...
        it("should position the tooltip with placement attribute value, when trigger is focused", () => {
            const component = testUtils.compileTemplate(
                '<div><button class="trigger" oui-tooltip="foo" oui-tooltip-placement="bottom-start"></button></div>'
            );
